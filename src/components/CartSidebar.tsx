...
export const CartSidebar: React.FC<CartSidebarProps> = ({ isOpen, onClose, onCheckout }) => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-[160] transform transition-transform duration-500 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
            >
                {/* Header */}
                    ...
                    {books.length === 0 ? (
                        ...
                        <div className="space-y-4">
                            {books.map((book) => (
                                <div
                                    key={book.id}
                                    className="group flex gap-4 p-3 rounded-2xl border border-gray-100 hover:border-primary-200 hover:bg-primary-50/30 transition-all duration-300"
                                >
